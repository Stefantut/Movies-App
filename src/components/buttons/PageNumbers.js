...
function PageNumbers({
  ...
  const nextPage = () => {
    ...
    switch (true) {
      case usedSearch:
        endpoint = `${BASE_URL}/search/movie?query=${searchQuery}&api_key=${API_KEY}&language=en-US&page=${
          currentPage + 1
        }`;
        break;
      case usedGenresFilter:
        endpoint = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=${
          selectedGenre.id
        }&page=${currentPage + 1}`;
        break;
      default:
        endpoint = `${BASE_URL}/discover/movie?sort_by=popularity.desc&api_key=${API_KEY}&page=${
          currentPage + 1
        }`;
    }
