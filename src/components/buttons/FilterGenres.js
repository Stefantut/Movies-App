...
const StyledButton = styled.button`
  ...
  padding: ${(props) => props.theme.spacers.xsmall}
    ${(props) => props.theme.spacers.medium};
...
function FilterGenres({
  ...
  const allGenres =
    ...
    genres.map((genre) => (
      <ListElement
        className={`genre genre--${convertToClassName(genre.name)} ${
  ...
  return (
    ...
    <StyledFilter>
      <StyledButton onClick={handleClick} visible={visible}>
        Filter by Genres<span>x</span>
      </StyledButton>
      {usedGenresFilter && (
        <StyledButton onClick={clearGenreFilter}>Clear Filter</StyledButton>
      )}

      <CSSTransition in={visible} timeout={250} classNames="fade" unmountOnExit>
        <List className="genres-list">{genresList}</List>
      </CSSTransition>
    </StyledFilter>
  );
}
